...
def encode_video(video_file, preprocess, model, batch_size, device, moment = None):
    ...
    if moment:
        ...
        start_frame = int(moment[0] * fps)
        end_frame = int(moment[1] * fps)
...
def em_cos_score(
    model, refs, hyps, ori_cands, ori_refs, vids, vid_feat_cache, tokenizer, idf_dict, preprocess, verbose=True, batch_size=64, device="cuda:0", return_matched_idx=False, moments = None
):
    """
    Compute EMScore.
    ...
    if vids:
        if vid_feat_cache:
            ...
            for vid_i in tqdm(range(len(unique_vids))):
                ...
                if moments:
                    video_file = vids[vid_i]
                    moment = moments[vid_i]
                    image_features, vid_feature = encode_video(video_file, preprocess, model, batch_size=512, device=device, moment=moment)    
                else:
    ...
    if vids:
        ...
        with torch.no_grad():
            for batch_start in iter_range: 
                ...
                vid_s_cogr = torch.bmm(ori_hyp_stats_global.unsqueeze(1), ori_vids_stats_global.unsqueeze(1).transpose(1, 2)).squeeze(dim=-1).squeeze(dim=-1)
