...
class CLIPTextEncoder(nn.Module):
    def __init__(self, joint_space_size, dataset):
        ...
        self.clip_model = self.clip_model.train().float()

        self.fc_out1 = nn.Linear(512, joint_space_size)
        self.fc_out2 = nn.Linear(512, joint_space_size)
        self.dataset = dataset
    ...
    def forward(self, queries, wordlens):
        ...
        sent_feat_iou = []
        for query, word_len in zip(queries, wordlens):  # each sample (several sentences) in a batch (of videos)
            clip_encoding = self.clip_model.encode_text(query)
            clip_encoding = F.normalize(clip_encoding.float(), dim=-1)
            query = self.layernorm(clip_encoding)
            out_iou = self.fc_out1(query)
            out = self.fc_out2(query)
...
def build_text_encoder(cfg):
    ...
    if cfg.DATASETS.FEATS_TYPE == 'original':
        text_encoder = DistilBert(joint_space_size, dataset_name)
    
    elif cfg.DATASETS.FEATS_TYPE == 'CLIP':
        text_encoder = CLIPTextEncoder(joint_space_size, dataset_name)
