...
def bert_embedding(cfg, sentence, tokenizer):
    # query_token = tokenizer(sentence, return_tensors="pt", padding=True)
    if cfg.DATASETS.NAME == "charades":
        max_len = 20
    ...
    query_token = tokenizer(sentence, return_tensors="pt", padding='max_length', max_length=max_len, truncation=True)
